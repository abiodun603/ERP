...
const defaultColumns: GridColDef[] = [
  {
    ...
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row.ticket}</Typography>
  },
  {
    flex: 0.25,
    field: 'title',
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => {
      return row.status.map((assignee: string, index: number) => (
        <CustomChip
          rounded
          size='small'
          key={index}
  ...
  },
  {
    ...
    headerName: 'Task',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row.task == '' ? '-' : row.task}</Typography>
    )
  },
  {
    flex: 0.25,
    field: 'millstone',
    minWidth: 100,
    headerName: 'Millstone',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row.millstone == '' ? '-' : row.millstone}</Typography>
    )
  },
  {
    flex: 0.25,
    ...
    headerName: 'Issues',
    renderCell: ({ row }: CellType) => (
      <Typography sx={{ color: 'text.secondary' }}>{row.issues == '' ? '-' : row.issues}</Typography>
    )
  },
  {
    flex: 0.25,
    ...
    renderCell: ({ row }: CellType) => <Typography sx={{ color: 'text.secondary' }}>{row.startDate}</Typography>
  },
  {
    flex: 0.25,
    minWidth: 100,
